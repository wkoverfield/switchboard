...
## Phase 2: Recommended Next Action

**Why second:** the product currently lists possible next steps. Humans need one
recommended next step, while agents/harnesses need structured alternatives.
...
6. Launch MCP or run command under the mandate.
...
## Phase 3: Code-Mode / CLI Authority

**Why third:** many coding agents will use shell commands, generated JS, SDKs,
...
Start with command class policy rather than full shell policy:

- allow common read-only provider commands for templates when known
- require approval for provider write commands
- deny known destructive forms where obvious
- otherwise mark command as `unclassified` and allow only with explicit
  `--allow-unclassified-command` or mandate setting
...
1. Publish patched npm release with import arg redaction.
2. Real Stripe test-mode dogfood:
...
3. Supabase dev design/proof:
   - no prod writes by default
   - database/schema/migration/storage policies
   - clear stance on data reads and PII
4. Optional Sentry/PostHog readonly templates after the money/database stories.
