...
- Added spawn-time auto-attenuation for Claude Code subagents, opt-in and reversible. `switchboard attenuation install claude` installs a PreToolUse hook that redirects a generic subagent spawn to a `scoped-worker` agent, whose inline MCP server launches `switchboard mcp --mint-child`: each spawn mints a fresh child pass under its parent and serves the scoped endpoint, so every subagent runs under its own id, its own audit trail, a lease bound to the parent, the inherited seatbelt floor and deny lists, and cascading revocation. It is off by default and `switchboard setup` does not enable it; install merges into user-scope Claude settings with a timestamped backup and writes `~/.claude/agents/scoped-worker.md`, and `switchboard attenuation uninstall claude` restores the pre-install state. The new `switchboard mcp --mint-child [--parent <id>]` verb mints and serves a child from `--parent`, then `SWITCHBOARD_PARENT_MANDATE`, then the repo's single active root pass, and exports the minted child id so a deeper spawn nests under the child (grandchild subset of child subset of root). `switchboard fleet` renders the delegation tree after a wave: who spawned whom, what each pass called, and what was denied. In v1 a child carries the SAME tool set as its parent; the enforced attenuation is identity, audit, floor, lease, and cascading revocation, not tool subsetting, and it is a policy layer, not an OS sandbox. Details in `docs/security/attenuation.md`.
- Added lazy per-call repo resolution to the daemon-backed MCP front door. One daemon serving a session that started at `~` now scopes each routed call to the repo the call is about to touch, not to the session's launch directory. When a call's arguments carry a filesystem path (a `path`, `file`, `dir`, `cwd`, `repo`, or `root`-style field whose value is an absolute or explicitly-relative path), the call resolves against that path's governing repo (nearest ancestor `.switchboard.yaml`, else nearest git root) and loads that repo's config, profiles, and pass. Precedence is explicit call path, then session cwd, then global default. A call with no derivable path resolves to the machine-level global config (the seatbelt floor still applies, repo profiles are simply not bound); it never prompts and never denies for lack of context, so the in-repo and single-repo flows are unchanged. Only path-shaped values under those keys count, so a bare token or an `owner/repo` slug is never mistaken for a checkout. Resolution composes under an only-strengthen rule: because a call's arguments are agent-controlled, a path argument that redirects a call to a different repo is evaluated against both the session context and the resolved repo and denied if either denies, so a session that is strict or pass-scoped keeps that governance no matter what path is supplied (an agent cannot escape its session's enforcement or pass scope by pointing a call at a permissive repo), and path arguments naming two different repos refuse to redirect rather than picking one by argument order. The seatbelt floor stays machine-scoped: a per-call binding can add repo restrictions but a repo `.switchboard.yaml` can never weaken the floor. Each routed audit entry now records `resolvedRepoPath` and `resolutionSource` (`call-path`, `session-cwd`, or `global-default`) so which repo governed a call is inspectable. Details in `docs/daemon.md`.
